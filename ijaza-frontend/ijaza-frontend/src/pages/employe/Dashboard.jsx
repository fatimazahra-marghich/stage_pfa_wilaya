...
export default function DashboardEmploye() {
  ...
  const acquis = solde ? solde.jours_acquis + solde.jours_reportes : 0;
  const pourcentage = acquis > 0 ? Math.max(0, Math.min(100, (restant / acquis) * 100)) : 0;

  return (
    <Layout>
      <h1 className="text-3xl font-bold mb-1">Bonjour, {user?.nomComplet?.split(" ")[0]}</h1>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
            ...
            <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
              <p className="text-sm text-neutral-500 mb-6">Acquis</p>
              ...
              <div key={d.id} className="flex items-center justify-between p-5">
                <div>
                  ...
                  <p className="text-sm text-neutral-500">
                    {d.date_debut} — {d.date_fin} ({d.nb_jours_ouvrables} jours)
                  </p>
                </div>
